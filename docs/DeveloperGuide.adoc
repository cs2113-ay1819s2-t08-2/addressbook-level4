...
===== Reason for implementation
To be able to keep track of one's progress with developing a habit, students should be able to add the habits they want to cultivate into the tracker along with their progress.
...
Step 1. The user launches the application for the first time. _LIFE_ will initialize with the initial habit tracker list state and the currentStatePointer pointing to that single purchase list state.
